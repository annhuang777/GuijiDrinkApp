...
class menu3TableViewController: UITableViewController {
    ...
    func fetchMenuRecords(){
        let urlStr = URL(string: "https://api.airtable.com/v0/apptv4BpzWCodIRFa/menu3")
        
        var request = URLRequest(url: urlStr!)
        request.setValue("Bearer \(apiKey)", forHTTPHeaderField: "Authorization")
        
        URLSession.shared.dataTask(with: request) { data, response, errir in
    ...
    @IBSegueAction func passingMenu3(_ coder: NSCoder) -> getOrderViewController? {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        as? menu3TableViewCell
        
        let menu = self.menuResponse?.records[indexPath.row]
